function ans=build(parameters)
	...
	%generating random edges in the matrix
	if isempty(parameters.adjacency_matrix)
		...
		for i = 1:path_size
			for j = [-1 1]
				...
				if i+j <= path_size && i+j > 0 && ceil((i+j)/parameters.path_height) == ceil(i/parameters.path_height)
					if parameters.edge_probability == 1
		...
		if issymmetric(parameters.adjacency_matrix)
			adjacency_mtx = parameters.adjacency_matrix;
		else
			error('wrong adjacency matrix is given');
		end
	end
	...
	%loading matrix data to the graph class
...
%% *PG graph object to display
	...
	pg = convert(PG);
	pg_f = floyd(pg);
	save(parameters.map_filename, 'PG', 'G', 'adjacency_mtx', 'vertices_pos', 'parameters', 'pg_f');
	ans = parameters.map_filename;
